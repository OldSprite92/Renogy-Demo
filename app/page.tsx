...
const scenes: Record<SceneKey, Scene> = {
  camp: {
    ...
    devices: [
      ...
      {
        icon: 'climate',
        name: { en: 'Climate', zh: '空调' },
        value: { en: 'Auto · 23°C', zh: '自动 · 23°C' },
        active: true,
      },
      {
        icon: 'humidifier',
        name: { en: 'Fresh air', zh: '新风' },
        value: { en: 'Balanced', zh: '均衡运行' },
        active: true,
      },
      {
        icon: 'inverter',
...
> = {
  camp: {
    ...
    factors: [
      ...
      {
        name: { en: 'Standby control', zh: '待机管理' },
        weight: 25,
        score: 88,
      },
      {
      ...
      {
        name: { en: 'Renewable use', zh: '清洁能源利用' },
        weight: 15,
        score: 88,
      },
    ],
  ...
  away: {
    ...
    actions: [
      {
        en: 'Cooking appliances are safely isolated',
        zh: '烹饪设备已安全断电',
      },
      { en: 'Solar charging has priority', zh: '太阳能充电处于优先级' },
  ...
  movie: {
    ...
    explanation: {
      en: 'Entertainment, spatial audio and performance power are active together while solar input is limited.',
  ...
  sleep: {
    ...
    actions: [
      {
        en: 'All cooking appliances are locked off',
        zh: '全部烹饪设备已锁定关闭',
      },
...
export default function Home() {
  ...
  const visualStateByDevice: Record<DeviceVisualKey, string> = {
    ...
    lock: getLoadByKey('lock').on ? 'locked' : 'unlocked',
  };
  ...
  const batteryFlowKw =
    Math.round(
      (Number.parseFloat(current.batteryFlow) - kitchenLoadWatts / 1000) * 100,
    ) / 100;
    ...
    <>
      <Sheet open={loadSheetOpen} onOpenChange={handleSheetOpenChange}>
        ...
        >
          ...
          <div className="dashboard">
              ...
              >
                <div className="panel-heading">
                  <div>
                    ...
                    <h2>{locale === 'en' ? 'Power flow' : '能量流'}</h2>
                ...
                <div className="forecast-card">
                  <div className="forecast-copy">
                    <span>
                      {locale === 'en' ? 'Estimated autonomy' : '预计续航'}
                    </span>
              ...
              <section className="hero-panel">
                ...
                <div className="scene-story">
                  <div className="scene-story-meta">
                    <span className="eyebrow">
                      {pick(current.name).toUpperCase()} MODE
                    </span>
                ...
                <fieldset className="vehicle-load-layer">
                  ...
                  {cabinLoads.map((load) => {
                    ...
                    return (
                      ...
                      >
                        <span className="actual-device-marker" aria-hidden="true" />
                ...
                {cameraOn ? (
                  <>
                    ...
                    <div className="readonly-note">
                      {locale === 'en'
                        ? 'Tap any camera to enlarge · Continuous recording'
                        : '点击任一画面放大查看 · 持续录像'}
                    </div>
                    <div className="security-grid">
                      <SecurityItem
                        icon={DoorClosed}
                        label={locale === 'en' ? 'Doors' : '车门'}
                        state={locale === 'en' ? 'Secured' : '已锁定'}
                        alert={false}
                      />
                      <SecurityItem
                        icon={ScanLine}
                      ...
                      <p>
                        ...
                        {temperatureSensor.on && airSensor.on
                          ? locale === 'en'
                            ...
                            : '温度与空气质量传感器在线'
                          : locale === 'en'
                            ? 'One or more environment sensors are offline'
                            : '环境传感器存在离线'}
                      </p>
                    </div>
            ...
            >
              ...
              <button
                ...
                aria-label={
                  locale === 'en'
                    ? `Energy optimization score ${activeEnergyInsight.score}, view details`
                    : `能源优化评分${activeEnergyInsight.score}分，查看详情`
                }
              >
                <Gauge aria-hidden="true" />
                <div>
                  <strong>
                    {activeEnergyInsight.score}
                    <span>{locale === 'en' ? '/100' : '分'}</span>
                  </strong>
                  <small>
                    {locale === 'en'
          ...
          <div className="portrait-gate">
            ...
            <p>
              {locale === 'en'
                ? 'Rotate your iPad for the full experience.'
                : '请将iPad旋转至横屏以获得完整体验。'}
            </p>
          </div>
          <SheetContent
      ...
      <Dialog open={efficiencyOpen} onOpenChange={setEfficiencyOpen}>
        <DialogContent className="efficiency-dialog" showCloseButton={false}>
          <DialogHeader className="efficiency-dialog-header">
            <div>
              <span className="eyebrow">RENOGY INTELLIGENT ENERGY</span>
              <DialogTitle>
                {locale === 'en' ? 'Energy optimization score' : '能源优化评分'}
              </DialogTitle>
              <DialogDescription>
                {locale === 'en'
                  ? 'How the current scene balances comfort, power and available energy.'
                  : '了解当前场景如何平衡舒适体验、负载功耗与可用能源。'}
              </DialogDescription>
            </div>
            <DialogClose
              className="efficiency-close"
              aria-label={
                locale === 'en' ? 'Close energy details' : '关闭能源详情'
              }
            >
              <X aria-hidden="true" />
            </DialogClose>
          ...
          <div className="efficiency-overview">
            <div className="efficiency-score-orbit">
              <Gauge aria-hidden="true" />
              <strong>
                {selectedEnergyInsight.score}
                <span>/100</span>
              </strong>
              <small>{pick(selectedEnergyInsight.grade)}</small>
            </div>
            <div className="efficiency-explanation">
              <span>
                {pick(scenes[efficiencyScene].name)}{' '}
                {locale === 'en' ? 'MODE' : '模式'}
              </span>
              <h3>{pick(selectedEnergyInsight.summary)}</h3>
              <p>{pick(selectedEnergyInsight.explanation)}</p>
            </div>
          </div>
          <div
            className="efficiency-scene-tabs"
            aria-label={
              locale === 'en' ? 'Compare scene scores' : '对比场景评分'
            }
          >
            {sceneOrder.map((key) => {
              const SceneIcon = scenes[key].sceneIcon;
              const selected = efficiencyScene === key;
              return (
                <button
                  type="button"
                  key={key}
                  className={selected ? 'is-selected' : ''}
                  onClick={() => setEfficiencyScene(key)}
                  aria-pressed={selected}
                >
                  <SceneIcon aria-hidden="true" />
                  <span>
                    <strong>{pick(scenes[key].name)}</strong>
                    <small>
                      {locale === 'en' ? 'View breakdown' : '查看构成'}
                    </small>
                  </span>
                  <b>{energyInsights[key].score}</b>
                </button>
              );
            })}
          </div>
            ...
            <div className="efficiency-section-heading">
              <div>
                ...
                <h3 id="efficiency-breakdown-title">
                  {locale === 'en' ? 'Score breakdown' : '评分构成'}
                </h3>
              </div>
              <small>{locale === 'en' ? 'Weighted total' : '加权计算'}</small>
            </div>
            <div className="efficiency-factor-grid">
              {selectedEnergyInsight.factors.map((factor) => (
                <div className="efficiency-factor" key={factor.name.en}>
                  <div>
                    <span>
                      {pick(factor.name)} <small>{factor.weight}%</small>
                    </span>
          ...
          <div className="efficiency-detail-grid">
            <section className="efficiency-actions">
              <span className="eyebrow">
                {locale === 'en' ? 'ACTIVE OPTIMIZATIONS' : '当前优化动作'}
              </span>
              {selectedEnergyInsight.actions.map((action) => (
                <p key={action.en}>
                  <Check aria-hidden="true" />
                  {pick(action)}
                </p>
            ...
            <section className="efficiency-opportunity">
              <Sparkles aria-hidden="true" />
              <div>
                <span>
                  {locale === 'en' ? 'NEXT OPPORTUNITY' : '下一步建议'}
                </span>
                <p>{pick(selectedEnergyInsight.opportunity)}</p>
              </div>
            </section>
          </div>
          <p className="efficiency-model-note">
            {locale === 'en'
              ? 'Concept score based on the current scene configuration and simulated demo data.'
              : '概念评分依据当前场景配置与模拟演示数据计算。'}
          </p>
        </DialogContent>
      </Dialog>
      <Dialog open={aiAdviceOpen} onOpenChange={setAiAdviceOpen}>
        <DialogContent className="ai-advice-dialog" showCloseButton={false}>
          <DialogHeader className="efficiency-dialog-header">
            <div>
              <span className="eyebrow">RENOGY AI ENERGY ADVISOR</span>
              <DialogTitle>
                {locale === 'en'
          ...
          >
            <div className="efficiency-section-heading">
              <div>
                <span className="eyebrow">WHAT YOUR BATTERY CAN SUPPORT</span>
                <h3 id="ai-capacity-title">
                  {locale === 'en'
                    ? 'Equivalent remaining activities'
                    : '剩余电量等效可支持'}
                </h3>
              </div>
              <small>
          ...
          <section className="ai-habit-plan">
            <div>
              ...
              <p>
                <Moon aria-hidden="true" />
                {locale === 'en' ? '8 h overnight comfort' : '8小时夜间舒适'}
          ...
          {selectedCamera && (
            <>
              <DialogHeader className="camera-dialog-header">
                <div>
                  <span className="eyebrow">
                    360° SENTINEL · CAM {selectedCamera.id}
                  </span>
                  <DialogTitle>
                    {locale === 'en'
              ...
              >
                ...
                <div className="camera-dialog-topline">
                  ...
                  <span>
                    CAM {selectedCamera.id} · {current.time}
                  </span>
                </div>
                {intrusion && selectedCamera.detectsIntrusion ? (
      ...
      case 'inverter':
        return (
          <>
            ...
            />
            <RangeControl
              label={t('AC output limit', 'AC输出上限')}
